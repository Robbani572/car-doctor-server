...
async function run() {
  try {
    ...
    const bookedService = client.db('carDoctor').collection('bookings')
    ...
    app.get('/bookings', async (req, res) => {
      ...
      const result = await bookedService.find(query).toArray()
      res.send(result)
    })

    app.post('/bookings', async (req, res) => {
      const bookings = req.body
      const result = await bookedService.insertOne(bookings);
      res.send(result)
    })
